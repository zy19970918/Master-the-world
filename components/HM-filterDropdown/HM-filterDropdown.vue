<template>
	<view class="HMfilterDropdown" :class="{'setDropdownBottom':maskVisibility}" :style="{'top':menuTop+'rpx'}"
	 @touchmove.stop.prevent="discard" @tap.stop="discard">
		...
		<view class="mask" :class="{'show':isShowMask,'hide':maskVisibility!=true}" @tap="togglePages(showPage)"></view>
		<block v-for="(page,page_index) in subData" :key="page_index">
			<view class="sub-menu-class" :class="{'show':showPage==page_index,'hide':pageState[page_index]!=true}">
				...
				<block v-if="page.type=='filter'">
					<view class="filter">
						<scroll-view class="menu-box" :scroll-y="true" style="">
							<!-- <text style="position: fe; top: 126rpx; left: 210rpx;">(50岁或50岁以上年龄段)</text> -->
							<!-- <view class="box"> -->
								<!-- <view class="title">最大年龄</view> -->
								<!-- <view class="labels"> -->
									<!-- <view class="" style="margin-right: 0;">
										<picker :class="{'on':flags}" id="btn" style="height: 100%; width: 100%; text-align: center; line-height: 30px;"
										 @change="bindPickerChange" :value="index" :range="array">
											{{array[index]}}岁
										</picker>
									</view>
									<text style="width: 24rpx; height: 1px; background-color:#333333; margin-top: 44rpx;"></text> -->
<!-- 									<view>
										<picker :class="{'on':flags}" style="height: 100%; width: 100%; text-align: center; line-height: 30px;"
										 @change="bindPickerChanges" :value="index" :range="arrays">
											{{arrays[indexs]}}岁
										</picker>
									</view>
 -->								<!-- 	<view class="" style="border: none; white-space: nowrap; color: red; margin-left: 30rpx;">
										(请选择工友年龄)
									</view> -->
								<!-- </view> -->
							<!-- </view> -->
					...
					if (typeof(item[0]) == 'object') {
						...
						item.forEach((s, j) => {
							if (s != null) {
								...
								item[j] = s;
								s.forEach((v, k) => {
									value[i][j][k] = (v == null || v >= this.subData[i].submenu[j].submenu.length) ? null : this.subData[i].submenu[
										j].submenu[v].value;
